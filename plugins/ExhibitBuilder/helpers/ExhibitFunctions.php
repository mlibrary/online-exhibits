 ...
 **/
 //Nancy update the function adding a condition to avoid empty tag for Video items
function exhibit_builder_thumbnail_gallery($start, $end, $props = array(), $thumbnailType = 'square_thumbnail') {

    $html = '';    
    for ($i = (int)$start; $i <= (int)$end; $i++) {
    //Nancy: Checking if there is an attachment file available if not do not add the div tag
        if (($attachment = exhibit_builder_page_attachment($i)) and ($attachment['file'])) {                    
