...
function exhibit_builder_generate_xml()
{
	$xml = new SimpleXMLElement('<xml/>');
//	while(loop_exhibits()):
	$currentexhibit = exhibit_builder_get_current_exhibit();
	$track = $xml->addChild('node');
...
function exhibit_builder_user_can_edit($exhibit = null, $user = null)
{
   ...
   // return ($canEditSelf || $canEditOthers);  
   //if ($exhibit['id']==5)  
   //$canEditOthers=1;
     
      return (($exhibit->wasAddedBy($user) && $canEditSelf) || $canEditOthers || (exhibit_builder_user_can_edit_groups($exhibit, $user) && $canEditSelf));
}
...
function exhibit_builder_user_can_delete($exhibit = null, $user = null)
{
    ...
    return (($exhibit->wasAddedBy($user) && $canDeleteSelf) || $canDeleteAll || (exhibit_builder_user_can_edit_groups($exhibit, $user) && $canDeleteSelf));
}
