    ...
    <?php foreach($exhibit->Sections as $key => $exhibitSection ): ?>
    	<li id="section_<?php echo html_escape($exhibitSection->id); ?>" class="exhibit-section-item">
			...
			<?php if (get_theme_option('View Items in Gallery')!='pages_2'):?>
						 <span class="page-add"><a class="add" href="<?php echo html_escape(uri('exhibits/add-page/'.$exhibitSection->id)); ?>"><?php echo __('Add a Page here'); ?></a></span>
			<?php elseif ((exhibit_builder_section_has_pages($exhibitSection))<1):?>
			 <span class="page-add"><a class="add" href="<?php echo html_escape(uri('exhibits/add-page/'.$exhibitSection->id)); ?>"><?php echo __('Add a Page here'); ?></a></span>
			 <?php endif?>
