...
<form method="post" id="choose-layout">
	...
	<fieldset id="layouts">
		...
		if ($exhibitPage->layout) {
			...
			if (get_theme_option('View Items in Gallery')=='pages_2')
			echo '<p>' . __('Enter the title of the Gallery and Choose the layout by selecting a thumbnail on the left.') . '</p>';
			else
		...
		<?php 
			...
			if (get_theme_option('View Items in Gallery')=='pages_2')
				echo exhibit_builder_exhibit_layout('gallery-thumbnails');
			else{
			foreach ($layouts as $layout) {
			if ($layout!='gallery-thumbnails')
