...
class CsvImport_Form_Mapping extends Omeka_Form
{
    ...
    public function init()
    {
        ...
        foreach ($this->_columnNames as $index => $colName) {
            ...
            $rowSubForm->addElement('checkbox', 'url');
    ...
    private function isFileMapped($index)
    {
     ...
     private function isReadFromUrlMapped($index)
    {
        return $this->getSubForm("row$index")->url->isChecked();
    }
    ...
    private function getColumnMap($index, $columnName)
    {
        ...
        } else if ($this->isReadFromUrlMapped($index)) {
            $columnMap = new CsvImport_ColumnMap_Url($columnName);
