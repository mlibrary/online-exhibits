...
class OpenLayersZoomPlugin extends Omeka_Plugin_AbstractPlugin
{
    ...
    public function hookAfterSaveItem($args)
    {
        ...
        $counter = 0;
        $file_objects = $post['open_layers_zoom_'];
        foreach ($file_objects as $objects => $value) {
	     ...
	     //     print_r("inside check box");
	      //    print_r($file);
	       ...
	       elseif (isset($value['hidden'])){// && ($filesaved != true)) {
		             $filesaved = false;
...
/* $html = '<script type="text/javascript">
    ...
    public function filterAdminItemsFormTabs($tabs, $args)
    {
        ...
        foreach($item->Files as $file) {
            if (strpos($file->mime_type, 'image/') === 0) {
                ...
                if ($this->isZoomed($file)) {
                    $isChecked = '<input type="checkbox" checked="checked" name="open_layers_zoom_[' . $file->id . '][checkbox]" id="open_layers_zoom_filename_' . $file->id . '" value="' . $file->filename . '"/>' . __('This image is zoomed.') . '</label>';
                    $isChecked .= '<input type="hidden" name="open_layers_zoom_[' . $file->id . '][hidden]" id="open_layers_zoom_removed_hidden_' . $file->id . '" value="' . $file->filename . '"/>';

                    $title = __('Click and Save Changes to make this image un zoom-able');
                    $style_color = "color:green";
                }
                else {
                    $isChecked = '<input type="checkbox" name="open_layers_zoom_[' . $file->id. '][checkbox]" id="open_layers_zoom_filename_' . $file->id . '" value="' . $file->filename . '"/>' . __('Zoom this image') . '</label>';
     ...
     * @return array
     */
    public function _getRootAndExtension($filepath)
    {
