<?php
//Groups table
class MlibraryGroupListOfGroups extends Omeka_Record_AbstractRecord {
    public $name;
    public $group_id;
...
function get_groups_names() {
  ...
  $group_list='';
  $groups='';
  $groups = $this->getTable('MlibraryGroupListOfGroups')->findAll();
  if(!empty($groups)) {
  ...
  $groups='';
   if (($role=='super') || ($role=='admin'))
       // $groups =  $db->getTable("CosignPage")->fetchObjects("SELECT * FROM {$db->prefix}groups");
        ...
        $groups =  $db->getTable("MlibraryGroupListOfGroups")->fetchObjects("SELECT * FROM {$db->prefix}groups g INNER JOIN {$db->prefix}grouping_relationship gr on g.group_id=gr.group_id where gr.entity_id=$id");
