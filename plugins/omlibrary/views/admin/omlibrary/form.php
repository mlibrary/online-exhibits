...
<?php if ( has_permission('Users','super') ): ?>
   <div class="field">
        <?php echo label('group','group'); ?>
        <div class="inputs">
    <?php 
	    $groupValue = ((!empty($user['entity_id'])) ? get_groups_ids_selected($user['entity_id'])  : '');
     //   $groupValue = get_groups_selected($user['id']);
	
       echo select(array('name'=>'group','id'=>'group', 'multiple'=>'multiple'),get_groups_names(),$groupValue); 
      // print_r($groupValue);
   //     echo radio(array('name'=>'groups', 'id'=>'groups'), array('0'=>'Inactive','1'=>'Active'), $user->active); 
    ?>
    </div>
