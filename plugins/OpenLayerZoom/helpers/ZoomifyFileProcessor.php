...
#    """ plan for the arrangement of the tile groups """
    ...
    function processRowImage($tier=0, $row=0) {
        ...
        list($root, $ext) = explode(".",basename(basename($this->_v_imageFilename)));
		 $directory = dirname($this->_v_imageFilename);
...
#        """ starting with the original image, start processing each row """
        ...
        $directory = dirname($this->_v_imageFilename);
          ...
          list($root,$ext)=explode(".",basename(basename($this->_v_imageFilename)));
       // list($root, $root2, $ext) = explode(".",$this->_v_imageFilename);
      //          $test = explode("/",$this->_v_imageFilename);
...
//            $saveFilename = "/www-dev/test.www.lib/web/sites/dev.lib.umich.edu/local/exhibits/archive/files/" . $tier . "-" . $row .  $ext;
...
print_r($this->_v_saveToLocation);
