	...
	**/
	function open_layer_zoom_add_zoom(file_name_base,width,height,url,req){
		...
		if (!open_layer_zoom_total_zooms){			
			jQuery(document.body).addClass('layerzoom'); // MLibrary TEMP - add class to body saying that this page has zooming on it, so css doesnt hide the blocks
			//yes so add the holders
			jQuery("#item-image").append(jQuery("<div>").attr("id",'open_layer_zoom_map'));
			jQuery("#item-image").append(jQuery("<div>").attr("id",'open_layer_zoom_map_more'));			
