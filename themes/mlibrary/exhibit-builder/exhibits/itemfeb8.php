...
{	
	...
	echo '<div id="slideshow"><div class="button-wrap"><div id="open-slideshow" class="button"><a href="#">open slideshow</a></div></div></div>';
...
<PARAM NAME="SRC" VALUE="'.uri('').'themes/'.$exhibit->theme.'/javascripts/ZoomifyViewer.swf">
<EMBED FlashVars="zoomifyImagePath='.uri('').'archive/zoom_tiles/'.$filename.'_zdata" SRC="'.uri('').'themes/'.$exhibit->theme.'/javascripts/ZoomifyViewer.swf" MENU="false" PLUGINSPAGE="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash"  WIDTH="550" HEIGHT="450" NAME="theMovie"></EMBED>
</OBJECT></div>';
	  }
	  else{
	...
	//  $html_fullsize_image.=display_file($file, array('imageSize'=>'fullsize','linkToFile'=>true,'linkAttributes'=>array('rel'=>'fancy_group', 'class'=>'fancyitem ' ,'title' => item('Dublin Core', 'Title'))),array('class' => 'fullsize img'.$image_index, 'style'=>'display:none', 'id' => 'item-image')); 
      $fullsizeimage=true;
	 ...
	 elseif ($fullsizeimage==true){ 
		...
		$filename = basename($file->archive_filename,'.jpg');  
  if(file_exists('archive/zoom_tiles/'.$filename.'_zdata')){   
        $html_fullsize_image .= '<div class="zoom img'.$image_index.' swf-zoom" style="display:none"><OBJECT CLASSID="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" CODEBASE="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0" WIDTH="750" HEIGHT="450" ID="theMovie">
<PARAM NAME="FlashVars" VALUE="zoomifyImagePath='.uri('').'archive/zoom_tiles/'.$filename.'_zdata">
<PARAM NAME="MENU" VALUE="FALSE">
<PARAM NAME="SRC" VALUE="'.uri('').'themes/'.$exhibit->theme.'/javascripts/ZoomifyViewer.swf">
<EMBED FlashVars="zoomifyImagePath='.uri('').'archive/zoom_tiles/'.$filename.'_zdata" SRC="'.uri('').'themes/'.$exhibit->theme.'/javascripts/ZoomifyViewer.swf" MENU="false" PLUGINSPAGE="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash"  WIDTH="550" HEIGHT="450" NAME="theMovie"></EMBED>
	    ...
	    $html_fullsize_image= display_file($file, array('imageSize'=>'fullsize','linkAttributes'=>array('class'=>'fancyitem ' ,'title' => item('Dublin Core', 'Title'))),array('class' => 'fullsize img'.$image_index, 'id' => 'item-image'));	        
}
	// $html_fullsize_image .=display_file($file, array('imageSize'=>'fullsize','linkToFile'=>true,'linkAttributes'=>array('rel'=>'fancy_group', 'class'=>'fancyitem' ,'title' => item('Dublin Core', 'Title'))),array('class' => 'fullsize img'.$image_index, 'style'=>'display:none', 'id' => 'item-image')); 
        $html_thumnailsize_image .=display_file($file, array('imageSize'=>'square_thumbnail', 'linkToFile'=>false),array('class' => 'square_thumbnail img'.$image_index )); 
    }
...
<!--  The following function prints all the the metadata associated with an item: Dublin Core, extra element sets, etc. See http://omeka.org/codex or the examples on items/browse for information on how to print only select metadata fields. -->
	...
	jQuery(".square_thumbnail").click(function(){    
		...
		//alert(imgClass);
		target.siblings(".fullsize , .zoom").hide();
        target.siblings(".file-metadata").hide();
        // look for imgClass and show it.
		target.siblings(".zoom."+imgClass).show();
