          ...
          if($file->hasThumbnail()) {  
          	  if ($fullsizeimage==false){	 
                	  $file_metadata = '<div class="file-metadata img'.$image_index.'">'.strip_formatting(metadata('file',array('Dublin Core', 'Title')))."</div>";                         	  
    		            ...
    		            $filename = basename($file->filename,'.jpg');	
    		          //   print_r($filename);
    		               if(file_exists('files/zoom_tiles/'.$filename.'_zdata')){ 
    		                	//$html_fullsize_image = '<div id="item-images">'.fire_plugin_hook('open_layers_zoom_display_file', array('file' => $file)).'</div>';
										      ...
										      <PARAM NAME="FlashVars" VALUE="zoomifyImagePath='.url('').'files/zoom_tiles/'.$filename.'_zdata">
										       ...
										       <PARAM NAME="SRC" VALUE="'.url('').'themes/'.$theme_name.'/javascripts/ZoomifyViewer.swf">
								         		...
								         		<EMBED FlashVars="zoomifyImagePath='.url('').'files/zoom_tiles/'.$filename.'_zdata" SRC="'.url('').'themes/'.$theme_name.
...
/*$index=0;
  ...
  endforeach;
   ...
   if (metadata('item', 'Collection Name')):
    ...
    <?php //Need to be done *******mlibrary_display_back_button_item_page('item'); ?>
    </div> <!-- end sidebar-->        
