...
{
  $html = "";
  $exhibits = get_records("Exhibit", ["sort_field" => "added", "sort_dir" => "d", "tags" => $tag], $recentExhibits);
  if ($exhibits) {
    foreach ($exhibits as $exhibit) {
      $title = metadata($exhibit, "title", ["snippet" => 300, "no_escape" => true]);
      $exhibitImage = record_image($exhibit, "original", ["alt" => "", "class" => "image-card"]);
      if ($exhibitImage == null) {
        $exhibitImage = '<img class="image-card" src="' . img("defaulthbg.jpg") . '" alt=""/>';
      }

      $html .= get_view()->partial("exhibit-builder/exhibits/card.php", ["exhibitImage" => $exhibitImage, "exhibit" => $exhibit, "title" => $title]);
    }
  } else {
...
function mlibrary_new_item_sequence($exhibitId, $from = null, $direction = null)
{
  ...
  $select = $itemTable
    ...
    ->joinInner(["attachments" => $db->ExhibitBlockAttachment], "`attachments`.`item_id` = `items`.`id`", ["block_id"])
    ->joinInner(["blocks" => $db->ExhibitPageBlock], "`attachments`.`block_id` = `blocks`.`id`", ["page_id"])
    ->joinInner(["pages" => $db->ExhibitPage], "`blocks`.`page_id` = `pages`.`id`", ["exhibit_id"])
    ->joinLeft(["parents" => $db->ExhibitPage], "`pages`.`parent_id` = `parents`.`id`", [])
...
function mlibrary_new_item_sequence_order($exhibitId, $pageId, $itemId)
{
  ...
  $select = $db
    ...
    ->columns(["sequence" => $sequence])
    ->join(["blocks" => $db->ExhibitPageBlock], "`blocks`.`page_id` = `pages`.`id`", [])
    ->join(["attachments" => $db->ExhibitBlockAttachment], "`attachments`.`block_id` = `blocks`.`id`", [])
    ->joinLeft(["parents" => $db->ExhibitPage], "`pages`.`parent_id` = `parents`.`id`", [])
    ->where("`pages`.`exhibit_id`    = ?", $exhibitId)
...
function mlibrary_new_display_popular_tags()
{
  ...
  $html = "<div>";
  if ($tagsExhibit) {
    $html .= get_view()->partial("exhibit-builder/exhibits/tags.php", ["tags" => $tagsExhibit]);
  } else {
...
function mlibrary_new_get_image_card($rawAttachment = null, $alt = "")
{
  ...
  if (empty($rawAttachment)) {
    return "<img class='image-card' alt='" . htmlentities($alt, ENT_QUOTES, "UTF-8") . "' src='{$default_image}'/>";
  }
  if (mlibrary_new_display_exhibit_type_of_item($rawAttachment) == "Video") {
    return mlibrary_new_exhibit_builder_video_attachment($rawAttachment->getItem(), $alt);
  }
  $file = $rawAttachment->getFile();
  if ($file->has_derivative_image) {
    $src = implode("/", [WEB_FILES, "fullsize", $file->getDerivativeFilename()]);
  } else {
    $src = implode("/", [WEB_FILES, "fullsize", $file->filename]);
  }
  return '<img src="' . htmlentities($src, ENT_QUOTES, "UTF-8") . '" alt="' . htmlentities($alt, ENT_QUOTES, "UTF-8") . '" class="image-card">';
}
...
function mlibrary_new_render_gallery_section($sectionpage_cards_info)
{
  return array_map(function ($sectionpage_card_info) {
    return '<div class="exhibit-gallery-theme-item panel panel-default">' . "<a href=" . $sectionpage_card_info["url"] . ">" . '<div class="panel-heading">' . $sectionpage_card_info["image"] . "</div>" . "</a>" . "</div>";
...
function mlibrary_new_exhibit_builder_display_random_featured_exhibit()
{
  ...
  if ($featuredExhibit) {
    $html .= get_view()->partial("exhibit-builder/exhibits/single.php", ["exhibit" => $featuredExhibit]);
  } else {
...
function mlibrary_new_display_video()
{
  ...
  $elementtitles = metadata(
    "item",
    ["Item Type Metadata", "Video Title"],
    [
      "no_escape" => true,
      "all" => true,
    ]
  ...
  if (!empty($elementvideos_VCM)) {
    ...
    foreach ($elementvideos_VCM as $i => $elementvideo_VCM) {
      if (empty($elementtitles[$i])) {
        $elementtitles[$i] = strip_formatting(metadata("item", ["Dublin Core", "Title"]));
      }
      $html_video .=
        "<div>" .
        $elementvideo_VCM .
    ...
    foreach ($elementvideos as $i => $elementvideo) {
      ...
      $html_video .=
        ...
        $elementvideo .
        '" frameborder="0" width="650" height="400"></iframe>
                   <div class="showcase-caption">
                   <h3>' .
        $elementtitles[$i] .
        '</h3>
...
function mlibrary_new_metadata($item)
{
  ...
  $elementInfos = [
    ...
    ["Dublin Core", "Source", "Item Source"],
    ["Dublin Core", "Description", "Description"],
    ["Dublin Core", "Identifier", "Source"],
    ["Dublin Core", "Publisher", "Publisher"],
    ["Dublin Core", "Contributor", "Contributor"],
    ["Dublin Core", "Rights", "Right"],
    ["Dublin Core", "Relation", "Relation"],
    ["Dublin Core", "Format", "Format"],
    ["Dublin Core", "Language", "Language"],
    ["Dublin Core", "Type", "Type"],
  ];
  ...
  foreach ($elementInfos as $elementInfo) {
    ...
    $elementTexts = metadata("item", [$elementSetName, $elementName], ["no_escape" => true, "all" => true]);
...
function mlibrary_new_display_rss($feedUrl, $num = 3)
{
  ...
  foreach ($feed as $entry) {
    ...
    echo "<p class='feed-title'><a href=\"$link\">$title</a></p>" . "<p class='feed-content'>$description <a href=\"$link\">...more</a></p>";
  }
}

function mlibrary_new_alt_text($item)
{
  return strip_formatting(metadata($item, ["Dublin Core", "Title"], ["no_escape" => true]));
}
...
{
  $remove = ["'"];
  $item_dublin_core_title = mlibrary_new_alt_text($item);
  $elementids_youtube_video = metadata($item, ["Item Type Metadata", "YouTube ID"], ["no_escape" => true, "all" => true]);
  $elementvideos_kultura_VCM = metadata($item, ["Item Type Metadata", "Embed Code"], ["no_escape" => true, "all" => true]);
  if (!empty($elementids_youtube_video)) {
    foreach ($elementids_youtube_video as $elementid_youtube_video) {
      ...
      if (!empty($videoid)) {
        return "<img class='image-card' src='//i.ytimg.com/vi/" . htmlentities($videoid, ENT_QUOTES, "UTF-8") . "/maxresdefault.jpg' alt='" . htmlentities($alt, ENT_QUOTES, "UTF-8") . "'>";
      }
    }
...
function mlibrary_new_exhibit_item_query_string_settings($exhibit_page = null)
{
  ...
  return ["exhibit" => $exhibit_page->exhibit_id, "page" => $exhibit_page->id];
}
...
{
  return "<li>" . '<a href="' . "#" . $exhibitPage["slug"] . '">' . metadata($exhibitPage, "title") . "</a>";
}
...
function mlibrary_new_render_exhibit_page($page)
{
  ...
  return mlibrary_new_add_vars_to_href(ob_get_clean(), mlibrary_new_exhibit_item_query_string_settings($page));
}
