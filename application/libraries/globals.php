...
function get_groups_names_belongto_user($id, $role){  
  ...
  $groups='';
	  ...
	  if (($role=='super') || ($role=='admin'))		
		$groups =  $db->getTable("Omlibrarygroups")->fetchObjects("SELECT * FROM {$db->prefix}groups");
	  else
  		$groups =  $db->getTable("Omlibrarygroups")->fetchObjects("SELECT * FROM {$db->prefix}groups g INNER JOIN {$db->prefix}grouping_relationship gr on g.group_id=gr.group_id where gr.entity_id=$id");
...
function get_groups_names(){  
  ...
  $group_list='';
  $groups='';
 // if(isset($db->getTable("Omlibrarygroups"))){
  	$groups =  $db->getTable("Omlibrarygroups")->fetchObjects("SELECT * FROM {$db->prefix}groups");
...
function get_group_name_by_group_id($group_id){
	$db = get_db();
	$groups='';
	if (!empty($group_id)){
    $groups =  $db->getTable("Omlibrarygroups")->fetchObjects("SELECT g.name FROM {$db->prefix}groups g where g.group_id = $group_id");                                                                                                                                  
...
function get_groups_ids_selected($id){
	 ...
	 $db = get_db();
	 if(!empty($id)){
     	$rows =  $db->getTable("Omlibrarygrouping")->fetchObjects("SELECT g.group_id FROM {$db->prefix}grouping_relationship g where g.entity_id = $id");
		if (!empty($rows)){
			foreach($rows as $row){
				$value[]= $row['group_id'];
			}
...
function test($group_exhibit, $user_id){
	 $db = get_db();
	 if((!empty($group_exhibit)) and (!empty($user_id))){
		 $rows =  $db->getTable("Omlibrarygrouping")->fetchObjects("SELECT * FROM {$db->prefix}grouping_relationship g where ((g.group_id=$group_exhibit) and (g.entity_id=$user_id))");                                                                
	}
	 //print_r($rows);                                                                
		 if (!empty($rows))
			 $result=1;
